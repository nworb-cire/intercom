...
The same adapter can remain a speaker-only sink (the default) or receive
authenticated microphone PCM over UDP when ``VOICE_PE_MICROPHONE=true``.
Speaker playback is provided by the core runtime's live FLAC sink.
"""
...
def create_source() -> VoicePeSource:
